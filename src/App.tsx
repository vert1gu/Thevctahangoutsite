...
import { ChevronRight, Download, MessageSquare, Sun, Moon, Gamepad2, Shield, Monitor, Github, Youtube, Search, Terminal, Globe, Zap, Check, Laptop, Sparkles, Menu, Cpu, Tv, AlertTriangle, Code2, Waves, Radio } from 'lucide-react';
...
const crackedGames = [
  ...
  { name: 'GOG-Games', url: 'https://gog-games.to/', description: 'DRM-free games from GOG.' },
];

const freeStreaming = [
  { name: 'Flixmomo', url: 'https://flixmomo.tv/', description: 'Free movies and TV shows streaming.' },
  { name: 'yFlix', url: 'https://yflix.to/home', description: 'Free movies and TV shows streaming.' },
...
const coolModesList = [
  ...
  { id: 'hacker', name: 'Hacker', icon: Code2, color: 'text-green-500 dark:text-green-500', bg: 'bg-green-500/10', border: 'border-green-500/20', desc: 'Enter the matrix.' },
  { id: 'ocean', name: 'Ocean', icon: Waves, color: 'text-blue-400 dark:text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/20', desc: 'Deep aquatic vibes.' },
  { id: 'synthwave', name: 'Synthwave', icon: Radio, color: 'text-pink-500 dark:text-pink-400', bg: 'bg-pink-500/10', border: 'border-pink-500/20', desc: '80s outrun aesthetic.' },
];
...
const SkeletonCard = () => (
  <div className="p-5 rounded-2xl mc:rounded-none tank:rounded-sm vertigo:rounded-3xl cyberpunk:rounded-none cyberpunk:bg-[#0d0221]/80 cyberpunk:border-fuchsia-900/50 retro:rounded-none retro:bg-[#2b2b40]/80 retro:border-purple-900/50 hacker:rounded-none hacker:bg-black hacker:border-green-900/50 ocean:rounded-3xl ocean:bg-[#001a33]/80 ocean:border-blue-900/50 synthwave:rounded-none synthwave:bg-[#1a0033]/80 synthwave:border-pink-900/50 bg-gray-50 dark:bg-[#161618] mc:bg-black/40 tank:bg-[#1a2315] vertigo:bg-white/5 border border-gray-200 dark:border-white/5 mc:border-white/20 tank:border-[#3a5a40] vertigo:border-white/10 vertigo:backdrop-blur-xl flex flex-col gap-3 animate-pulse">
    <div className="flex items-center justify-between w-full">
      <div className="h-5 w-32 bg-gray-200 dark:bg-white/10 mc:bg-white/20 tank:bg-[#3a5a40] vertigo:bg-white/20 cyberpunk:bg-fuchsia-900/50 cyberpunk:rounded-none retro:bg-purple-900/50 retro:rounded-none hacker:bg-green-900/50 hacker:rounded-none ocean:bg-blue-900/50 ocean:rounded-3xl synthwave:bg-pink-900/50 synthwave:rounded-none rounded-md mc:rounded-none tank:rounded-sm vertigo:rounded-lg" />
      <div className="h-4 w-4 bg-gray-200 dark:bg-white/10 mc:bg-white/20 tank:bg-[#3a5a40] vertigo:bg-white/20 cyberpunk:bg-fuchsia-900/50 cyberpunk:rounded-none retro:bg-purple-900/50 retro:rounded-none hacker:bg-green-900/50 hacker:rounded-none ocean:bg-blue-900/50 ocean:rounded-full synthwave:bg-pink-900/50 synthwave:rounded-none rounded-full mc:rounded-none tank:rounded-sm vertigo:rounded-full" />
    </div>
    <div className="space-y-2">
      <div className="h-3 w-full bg-gray-200 dark:bg-white/10 mc:bg-white/20 tank:bg-[#3a5a40] vertigo:bg-white/20 cyberpunk:bg-fuchsia-900/50 cyberpunk:rounded-none retro:bg-purple-900/50 retro:rounded-none hacker:bg-green-900/50 hacker:rounded-none ocean:bg-blue-900/50 ocean:rounded-3xl synthwave:bg-pink-900/50 synthwave:rounded-none rounded-md mc:rounded-none tank:rounded-sm vertigo:rounded-lg" />
      <div className="h-3 w-2/3 bg-gray-200 dark:bg-white/10 mc:bg-white/20 tank:bg-[#3a5a40] vertigo:bg-white/20 cyberpunk:bg-fuchsia-900/50 cyberpunk:rounded-none retro:bg-purple-900/50 retro:rounded-none hacker:bg-green-900/50 hacker:rounded-none ocean:bg-blue-900/50 ocean:rounded-3xl synthwave:bg-pink-900/50 synthwave:rounded-none rounded-md mc:rounded-none tank:rounded-sm vertigo:rounded-lg" />
    </div>
  </div>
);
...
export default function App() {
  ...
  const [appMode, setAppMode] = useState<'default' | 'mc' | 'tank' | 'vertigo' | 'lemon' | 'burger' | 'thinkpad' | 'linux' | 'cyberpunk' | 'retro' | 'error404' | 'hacker' | 'ocean' | 'synthwave'>(() => {
    if (typeof window !== 'undefined') {
  ...
  const handleModeChange = (newMode: 'default' | 'mc' | 'tank' | 'vertigo' | 'lemon' | 'burger' | 'thinkpad' | 'linux' | 'cyberpunk' | 'retro' | 'error404' | 'hacker' | 'ocean' | 'synthwave') => {
    if (newMode === appMode) {
  ...
  useEffect(() => {
    document.documentElement.classList.remove('mc-mode', 'tank-mode', 'vertigo-mode', 'lemon-mode', 'burger-mode', 'thinkpad-mode', 'linux-mode', 'cyberpunk-mode', 'retro-mode', 'error404-mode', 'hacker-mode', 'ocean-mode', 'synthwave-mode');
    if (appMode === 'mc') {
    ...
    } else if (appMode === 'hacker') {
      document.documentElement.classList.add('hacker-mode', 'dark');
    } else if (appMode === 'ocean') {
      document.documentElement.classList.add('ocean-mode', 'dark');
    } else if (appMode === 'synthwave') {
      document.documentElement.classList.add('synthwave-mode', 'dark');
    } else {
      if (theme === 'dark') {
    ...
    <div className="min-h-screen selection:bg-indigo-500/30">
      ...
      <nav className="fixed top-0 w-full z-50 border-b border-gray-200 dark:border-white/5 bg-white/80 dark:bg-[#050505]/80 mc:bg-black/40 mc:border-white/10 tank:bg-[#11140d]/90 tank:border-[#3a5a40]/50 vertigo:bg-white/5 vertigo:backdrop-blur-2xl vertigo:border-white/10 cyberpunk:bg-[#0d0221]/80 cyberpunk:border-fuchsia-500/30 retro:bg-[#2b2b40]/90 retro:border-purple-500/30 backdrop-blur-md transition-colors duration-200">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="font-display font-bold text-xl tracking-tight text-gray-900 dark:text-white mc:text-2xl tank:text-2xl tank:uppercase tank:tracking-widest tank:text-[#dad7cd] vertigo:text-2xl vertigo:tracking-tighter vertigo:font-medium cyberpunk:text-fuchsia-400 cyberpunk:tracking-widest retro:text-purple-400 retro:tracking-widest error404:text-red-500 hacker:text-green-500">X.KERO.D 'S Site</div>
          <div className="flex items-center gap-4">
            <div className="relative group flex items-center justify-center">
              <button
                onClick={() => setIsModeMenuOpen(!isModeMenuOpen)}
                className={`p-2 transition-colors rounded-full mc:rounded-none tank:rounded-sm vertigo:rounded-2xl ${appMode !== 'default' ? 'bg-black/40 border vertigo:bg-white/10 vertigo:border-white/20 lemon:bg-yellow-200 lemon:border-yellow-400 burger:bg-amber-100 burger:border-amber-300 thinkpad:bg-black thinkpad:border-red-600 linux:bg-black linux:border-green-500 cyberpunk:bg-[#0d0221] cyberpunk:border-fuchsia-500 retro:bg-[#2b2b40] retro:border-purple-500 error404:bg-red-900/20 error404:border-red-500 hacker:bg-black hacker:border-green-500 ocean:bg-blue-900/20 ocean:border-blue-500 synthwave:bg-[#1a0033] synthwave:border-pink-500' : 'hover:bg-gray-100 dark:hover:bg-white/5'} ${appMode === 'mc' ? 'text-green-400 border-green-500/30' : appMode === 'tank' ? 'text-[#a3b18a] border-[#3a5a40]' : appMode === 'vertigo' ? 'text-cyan-400 border-cyan-500/30' : appMode === 'lemon' ? 'text-yellow-600 border-yellow-400' : appMode === 'burger' ? 'text-amber-800 border-amber-400' : appMode === 'thinkpad' ? 'text-red-500 border-red-600' : appMode === 'linux' ? 'text-green-500 border-green-500' : appMode === 'cyberpunk' ? 'text-fuchsia-500 border-fuchsia-500' : appMode === 'retro' ? 'text-purple-400 border-purple-500' : appMode === 'error404' ? 'text-red-500 border-red-500' : appMode === 'hacker' ? 'text-green-500 border-green-500' : appMode === 'ocean' ? 'text-blue-400 border-blue-500' : appMode === 'synthwave' ? 'text-pink-500 border-pink-500' : 'text-gray-500 dark:text-gray-400'} focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 mc:focus-visible:ring-green-500 tank:focus-visible:ring-[#a3b18a] vertigo:focus-visible:ring-cyan-500 cyberpunk:focus-visible:ring-fuchsia-500 retro:focus-visible:ring-purple-500 error404:focus-visible:ring-red-500 hacker:focus-visible:ring-green-500 ocean:focus-visible:ring-blue-500 synthwave:focus-visible:ring-pink-500`}
              ...
              >
                ...
                {appMode === 'hacker' && <Code2 className="w-5 h-5 transition-transform duration-200 group-hover:scale-110 text-green-500" aria-hidden="true" />}
                {appMode === 'ocean' && <Waves className="w-5 h-5 transition-transform duration-200 group-hover:scale-110 text-blue-400" aria-hidden="true" />}
                {appMode === 'synthwave' && <Radio className="w-5 h-5 transition-transform duration-200 group-hover:scale-110 text-pink-500" aria-hidden="true" />}
              </button>
                ...
                {isModeMenuOpen && (
                  ...
                  >
                    ...
                    <button 
                      ...
                      role="menuitem"
                      className={`px-3 py-2 text-left text-sm rounded-lg mc:rounded-none tank:rounded-sm vertigo:rounded-xl flex items-center justify-between gap-2 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 mc:focus-visible:ring-green-500 tank:focus-visible:ring-[#a3b18a] vertigo:focus-visible:ring-cyan-500 cyberpunk:focus-visible:ring-fuchsia-500 retro:focus-visible:ring-purple-500 error404:focus-visible:ring-red-500 hacker:focus-visible:ring-green-500 ocean:focus-visible:ring-blue-500 synthwave:focus-visible:ring-pink-500 ${appMode === 'error404' ? 'bg-red-900/40 text-red-500' : 'hover:bg-gray-50 dark:hover:bg-white/5 retro:hover:bg-white/10 text-gray-700 dark:text-gray-300 mc:text-green-400 tank:text-[#a3b18a] vertigo:text-cyan-400 lemon:text-yellow-700 burger:text-amber-800 thinkpad:text-white linux:text-green-500 cyberpunk:text-fuchsia-500 retro:text-purple-400 error404:text-red-500 hacker:text-green-500 ocean:text-blue-400 synthwave:text-pink-500'}`}
                    >
                      <div className="flex items-center gap-2">
                    ...
                    <button 
                      ...
                      role="menuitem"
                      className={`px-3 py-2 text-left text-sm rounded-lg mc:rounded-none tank:rounded-sm vertigo:rounded-xl flex items-center justify-between gap-2 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 mc:focus-visible:ring-green-500 tank:focus-visible:ring-[#a3b18a] vertigo:focus-visible:ring-cyan-500 cyberpunk:focus-visible:ring-fuchsia-500 retro:focus-visible:ring-purple-500 error404:focus-visible:ring-red-500 hacker:focus-visible:ring-green-500 ocean:focus-visible:ring-blue-500 synthwave:focus-visible:ring-pink-500 ${appMode === 'hacker' ? 'bg-green-900/40 text-green-500' : 'hover:bg-gray-50 dark:hover:bg-white/5 retro:hover:bg-white/10 text-gray-700 dark:text-gray-300 mc:text-green-400 tank:text-[#a3b18a] vertigo:text-cyan-400 lemon:text-yellow-700 burger:text-amber-800 thinkpad:text-white linux:text-green-500 cyberpunk:text-fuchsia-500 retro:text-purple-400 error404:text-red-500 hacker:text-green-500 ocean:text-blue-400 synthwave:text-pink-500'}`}
                    >
                      <div className="flex items-center gap-2">
                        <Code2 className="w-4 h-4 text-green-500" aria-hidden="true" /> <span className="text-green-500">Hacker</span>
                      </div>
                      {appMode === 'hacker' && <Check className="w-4 h-4 text-green-500" aria-hidden="true" />}
                    </button>
                    <button 
                      onClick={() => handleModeChange('ocean')} 
                      role="menuitem"
                      className={`px-3 py-2 text-left text-sm rounded-lg mc:rounded-none tank:rounded-sm vertigo:rounded-xl flex items-center justify-between gap-2 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 mc:focus-visible:ring-green-500 tank:focus-visible:ring-[#a3b18a] vertigo:focus-visible:ring-cyan-500 cyberpunk:focus-visible:ring-fuchsia-500 retro:focus-visible:ring-purple-500 error404:focus-visible:ring-red-500 hacker:focus-visible:ring-green-500 ocean:focus-visible:ring-blue-500 synthwave:focus-visible:ring-pink-500 ${appMode === 'ocean' ? 'bg-blue-900/40 text-blue-400' : 'hover:bg-gray-50 dark:hover:bg-white/5 retro:hover:bg-white/10 text-gray-700 dark:text-gray-300 mc:text-green-400 tank:text-[#a3b18a] vertigo:text-cyan-400 lemon:text-yellow-700 burger:text-amber-800 thinkpad:text-white linux:text-green-500 cyberpunk:text-fuchsia-500 retro:text-purple-400 error404:text-red-500 hacker:text-green-500 ocean:text-blue-400 synthwave:text-pink-500'}`}
                    >
                      <div className="flex items-center gap-2">
                        <Waves className="w-4 h-4 text-blue-400" aria-hidden="true" /> <span className="text-blue-400">Ocean</span>
                      </div>
                      {appMode === 'ocean' && <Check className="w-4 h-4 text-blue-400" aria-hidden="true" />}
                    </button>
                    <button 
                      onClick={() => handleModeChange('synthwave')} 
                      role="menuitem"
                      className={`px-3 py-2 text-left text-sm rounded-lg mc:rounded-none tank:rounded-sm vertigo:rounded-xl flex items-center justify-between gap-2 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 mc:focus-visible:ring-green-500 tank:focus-visible:ring-[#a3b18a] vertigo:focus-visible:ring-cyan-500 cyberpunk:focus-visible:ring-fuchsia-500 retro:focus-visible:ring-purple-500 error404:focus-visible:ring-red-500 hacker:focus-visible:ring-green-500 ocean:focus-visible:ring-blue-500 synthwave:focus-visible:ring-pink-500 ${appMode === 'synthwave' ? 'bg-pink-900/40 text-pink-500' : 'hover:bg-gray-50 dark:hover:bg-white/5 retro:hover:bg-white/10 text-gray-700 dark:text-gray-300 mc:text-green-400 tank:text-[#a3b18a] vertigo:text-cyan-400 lemon:text-yellow-700 burger:text-amber-800 thinkpad:text-white linux:text-green-500 cyberpunk:text-fuchsia-500 retro:text-purple-400 error404:text-red-500 hacker:text-green-500 ocean:text-blue-400 synthwave:text-pink-500'}`}
                    >
                      <div className="flex items-center gap-2">
                        <Radio className="w-4 h-4 text-pink-500" aria-hidden="true" /> <span className="text-pink-500">Synthwave</span>
                      </div>
                      {appMode === 'synthwave' && <Check className="w-4 h-4 text-pink-500" aria-hidden="true" />}
                    </button>
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            <a 
              href="https://discord.gg/ZXg9eUdc7f" 
              target="_blank" 
              rel="noreferrer"
              className="group flex items-center gap-2 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 error404:text-red-500 error404:hover:text-red-400 hacker:text-green-500 hacker:hover:text-green-400 transition-colors"
            >
      ...
      <section className="relative pt-32 pb-20 px-6 min-h-[80vh] flex flex-col items-center justify-center overflow-hidden">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full mc:rounded-none mc:bg-black/60 mc:border-green-500/50 mc:text-green-400 tank:rounded-sm tank:bg-[#1a2315] tank:border-[#588157] tank:text-[#a3b18a] tank:uppercase tank:tracking-widest cyberpunk:rounded-none cyberpunk:bg-fuchsia-900/30 cyberpunk:border-fuchsia-500/50 cyberpunk:text-fuchsia-400 retro:rounded-none retro:bg-purple-900/30 retro:border-purple-500/50 retro:text-purple-400 error404:rounded-none error404:bg-red-900/30 error404:border-red-500/50 error404:text-red-500 hacker:rounded-none hacker:bg-green-900/30 hacker:border-green-500/50 hacker:text-green-500 bg-indigo-50 dark:bg-white/5 border border-indigo-100 dark:border-white/10 text-sm font-medium mb-8 text-indigo-600 dark:text-indigo-300 transition-colors duration-200">
            <span className="w-2 h-2 rounded-full mc:rounded-none mc:bg-green-500 tank:rounded-none tank:bg-[#a3b18a] cyberpunk:rounded-none cyberpunk:bg-fuchsia-500 retro:rounded-none retro:bg-purple-500 error404:rounded-none error404:bg-red-500 hacker:rounded-none hacker:bg-green-500 bg-indigo-500 animate-pulse" />
            Tech, Community & Hangout
          </div>
          
          <h1 className="font-display text-5xl md:text-7xl font-bold tracking-tight mb-6 leading-tight text-gray-900 dark:text-white error404:text-red-500 hacker:text-green-500 transition-colors duration-200">
            Welcome to <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-cyan-500 dark:from-indigo-400 dark:to-cyan-400 mc:from-green-400 mc:to-emerald-500 tank:from-[#a3b18a] tank:to-[#dad7cd] tank:uppercase tank:tracking-wider cyberpunk:from-fuchsia-500 cyberpunk:to-cyan-400 retro:from-purple-400 retro:to-pink-500 error404:from-red-500 error404:to-red-700 hacker:from-green-400 hacker:to-green-600">
              X.KERO.D 'S Site
            </span>
          </h1>
          
          <p className="text-lg md:text-xl text-gray-600 dark:text-gray-400 mc:text-gray-300 tank:text-[#8a9a70] cyberpunk:text-fuchsia-200/70 retro:text-purple-200/70 error404:text-red-500/80 hacker:text-green-500/80 mb-10 max-w-2xl mx-auto leading-relaxed transition-colors duration-200">
            X.KERO.D 'S Site for everything, discord, resources, cool modes and everything
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <a 
              ...
              target="_blank" 
              rel="noreferrer"
              className="w-full sm:w-auto px-8 py-4 rounded-full mc:rounded-none mc:bg-green-600 mc:hover:bg-green-500 mc:shadow-none tank:rounded-sm tank:bg-[#3a5a40] tank:hover:bg-[#588157] tank:shadow-none tank:border tank:border-[#a3b18a]/30 tank:uppercase tank:tracking-wider cyberpunk:rounded-none cyberpunk:bg-fuchsia-600 cyberpunk:hover:bg-fuchsia-500 cyberpunk:shadow-[0_0_30px_-5px_rgba(217,70,239,0.4)] retro:rounded-none retro:bg-purple-600 retro:hover:bg-purple-500 retro:shadow-[0_0_30px_-5px_rgba(168,85,247,0.4)] error404:rounded-none error404:bg-red-900 error404:text-red-500 error404:border error404:border-red-500 error404:hover:bg-red-800 hacker:rounded-none hacker:bg-green-900 hacker:text-green-500 hacker:border hacker:border-green-500 hacker:hover:bg-green-800 bg-indigo-600 hover:bg-indigo-500 text-white font-medium transition-all flex items-center justify-center gap-2 shadow-[0_0_30px_-5px_rgba(79,70,229,0.3)] dark:shadow-[0_0_30px_-5px_rgba(79,70,229,0.4)]"
            >
            ...
            <button 
              ...
              className="w-full sm:w-auto px-8 py-4 rounded-full mc:rounded-none mc:bg-black/60 mc:border-white/20 mc:hover:bg-black/80 tank:rounded-sm tank:bg-[#11140d] tank:border-[#3a5a40] tank:hover:bg-[#1a2315] tank:text-[#a3b18a] tank:uppercase tank:tracking-wider cyberpunk:rounded-none cyberpunk:bg-[#0d0221] cyberpunk:border-fuchsia-500/50 cyberpunk:hover:bg-[#1a0533] cyberpunk:text-fuchsia-400 retro:rounded-none retro:bg-[#2b2b40] retro:border-purple-500/50 retro:hover:bg-[#363650] retro:text-purple-400 error404:rounded-none error404:bg-black error404:border-red-500/50 error404:text-red-500 error404:hover:bg-red-900/20 hacker:rounded-none hacker:bg-black hacker:border-green-500/50 hacker:text-green-500 hacker:hover:bg-green-900/20 bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white font-medium transition-all flex items-center justify-center gap-2"
            >
              Explore Resources
      ...
      <section ref={exploreRef} className="py-24 px-6 relative">
        <div className="max-w-5xl mx-auto">
          <div className="bg-white dark:bg-[#0f0f11] mc:bg-black/60 mc:backdrop-blur-md mc:border-white/10 mc:rounded-none tank:bg-[#11140d]/90 tank:backdrop-blur-lg tank:border-2 tank:border-[#3a5a40] tank:rounded-sm rounded-[2rem] p-8 md:p-12 shadow-xl dark:shadow-none border border-gray-200 dark:border-white/5 cyberpunk:bg-[#0d0221]/80 cyberpunk:backdrop-blur-md cyberpunk:border-fuchsia-500/30 cyberpunk:rounded-none retro:bg-[#2b2b40]/90 retro:backdrop-blur-md retro:border-purple-500/30 retro:rounded-none error404:bg-black error404:border-red-900/50 error404:rounded-none hacker:bg-black hacker:border-green-900/50 hacker:rounded-none transition-colors duration-200">
            {/* Header */}
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-10 gap-6">
              <h2 className="font-display text-2xl md:text-3xl font-bold text-gray-900 dark:text-white tank:uppercase tank:tracking-wider tank:text-[#dad7cd] cyberpunk:text-fuchsia-400 retro:text-purple-400 error404:text-red-500 hacker:text-green-500">Downloads & Resources</h2>
              
              {/* Pill Toggle */}
              <div 
                className="inline-flex flex-wrap justify-center p-1.5 bg-gray-100 dark:bg-[#050505] mc:bg-black/40 mc:border-white/10 mc:rounded-none tank:bg-[#1a2315] tank:border-[#3a5a40] tank:rounded-sm rounded-3xl md:rounded-full border border-gray-200 dark:border-white/5 cyberpunk:bg-[#1a0533]/50 cyberpunk:border-fuchsia-500/20 cyberpunk:rounded-none retro:bg-[#363650]/50 retro:border-purple-500/20 retro:rounded-none error404:bg-red-900/10 error404:border-red-900/30 error404:rounded-none hacker:bg-green-900/10 hacker:border-green-900/30 hacker:rounded-none transition-colors duration-200 gap-1 relative"
                role="tablist"
              ...
              >
                {[
                  ...
                  <button
                    ...
                    className={`relative px-6 py-2.5 rounded-full mc:rounded-none tank:rounded-sm tank:uppercase tank:tracking-wider text-sm font-medium transition-colors duration-200 z-10 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 mc:focus-visible:ring-green-500 tank:focus-visible:ring-[#a3b18a] vertigo:focus-visible:ring-cyan-500 cyberpunk:rounded-none cyberpunk:focus-visible:ring-fuchsia-500 retro:rounded-none retro:focus-visible:ring-purple-500 error404:rounded-none error404:focus-visible:ring-red-500 hacker:rounded-none hacker:focus-visible:ring-green-500 ${
                      activeTab === tab.id 
                        ? 'text-white tank:text-[#11140d]' 
                        : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 hover:bg-gray-200/50 dark:hover:bg-white/5 mc:hover:bg-white/10 tank:hover:bg-[#3a5a40]/40 tank:text-[#8a9a70] cyberpunk:hover:bg-fuchsia-900/30 cyberpunk:text-fuchsia-500/70 retro:hover:bg-purple-900/30 retro:text-purple-500/70 error404:hover:bg-red-900/30 error404:text-red-500/70 hacker:hover:bg-green-900/30 hacker:text-green-500/70'
                    }`}
                  >
                    {activeTab === tab.id && (
                      <motion.div
                        layoutId="activeTab"
                        className="absolute inset-0 bg-indigo-600 mc:bg-green-600 tank:bg-[#a3b18a] cyberpunk:bg-fuchsia-600 cyberpunk:rounded-none retro:bg-purple-600 retro:rounded-none error404:bg-red-600 error404:rounded-none hacker:bg-green-600 hacker:rounded-none rounded-full mc:rounded-none tank:rounded-sm shadow-md -z-10"
      ...
      <section className="py-24 px-6 relative bg-gray-100/50 dark:bg-[#0a0a0a]/50 mc:bg-black/80 tank:bg-[#0f120b]/80 vertigo:bg-[#020617]/50 cyberpunk:bg-[#0d0221]/90 retro:bg-[#2b2b40]/90 error404:bg-black hacker:bg-[#001100] ocean:bg-[#000a1a] synthwave:bg-[#0a001a] transition-colors duration-200">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="font-display text-3xl md:text-4xl font-bold text-gray-900 dark:text-white tank:uppercase tank:tracking-wider tank:text-[#dad7cd] cyberpunk:text-fuchsia-400 retro:text-purple-400 error404:text-red-500 hacker:text-green-500 ocean:text-blue-400 synthwave:text-pink-500 mb-4">Discover Cool Modes</h2>
            <p className="text-lg text-gray-600 dark:text-gray-400 mc:text-gray-300 tank:text-[#8a9a70] cyberpunk:text-fuchsia-200/70 retro:text-purple-200/70 error404:text-red-500/70 hacker:text-green-500/70 ocean:text-blue-400/70 synthwave:text-pink-500/70 max-w-2xl mx-auto">
            ...
            {coolModesList.map((mode) => {
              ...
              return (
                <div 
                  ...
                  className={`group cursor-pointer p-6 rounded-2xl mc:rounded-none tank:rounded-sm vertigo:rounded-3xl cyberpunk:rounded-none retro:rounded-none error404:rounded-none hacker:rounded-none ocean:rounded-3xl synthwave:rounded-none border transition-all duration-300 flex flex-col items-center text-center gap-4 ${isActive ? 'bg-white dark:bg-[#161618] mc:bg-black/60 tank:bg-[#1a2315] vertigo:bg-white/10 cyberpunk:bg-[#1a0533] retro:bg-[#363650] error404:bg-red-900/20 hacker:bg-green-900/20 ocean:bg-blue-900/20 synthwave:bg-pink-900/20 shadow-lg scale-105 border-indigo-500 mc:border-green-500 tank:border-[#a3b18a] vertigo:border-cyan-500 lemon:border-yellow-500 burger:border-amber-500 thinkpad:border-red-600 linux:border-green-500 cyberpunk:border-fuchsia-500 retro:border-purple-500 error404:border-red-500 hacker:border-green-500 ocean:border-blue-500 synthwave:border-pink-500' : 'bg-white/50 dark:bg-[#161618]/50 mc:bg-black/40 tank:bg-[#1a2315]/50 vertigo:bg-white/5 cyberpunk:bg-[#1a0533]/50 cyberpunk:border-fuchsia-900/50 cyberpunk:hover:bg-[#1a0533] retro:bg-[#363650]/50 retro:border-purple-900/50 retro:hover:bg-[#363650] error404:bg-black/50 error404:border-red-900/50 error404:hover:bg-red-900/10 hacker:bg-black/50 hacker:border-green-900/50 hacker:hover:bg-green-900/10 ocean:bg-[#001a33]/50 ocean:border-blue-900/50 ocean:hover:bg-[#001a33] synthwave:bg-[#1a0033]/50 synthwave:border-pink-900/50 synthwave:hover:bg-[#1a0033] border-gray-200 dark:border-white/5 mc:border-white/10 tank:border-[#3a5a40] vertigo:border-white/10 hover:bg-white dark:hover:bg-[#161618] hover:shadow-md hover:-translate-y-1'}`}
                >
                  <div className={`w-16 h-16 rounded-2xl mc:rounded-none tank:rounded-sm vertigo:rounded-full cyberpunk:rounded-none retro:rounded-none error404:rounded-none hacker:rounded-none ocean:rounded-full synthwave:rounded-none flex items-center justify-center ${mode.bg} ${mode.color} ${mode.border} border transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3`}>
                    <Icon className="w-8 h-8" />
                  </div>
                  <div>
                    <h3 className={`font-display font-bold text-lg mb-1 ${isActive ? mode.color : 'text-gray-900 dark:text-white tank:text-[#dad7cd] cyberpunk:text-fuchsia-200 retro:text-purple-200 error404:text-red-500 hacker:text-green-500 ocean:text-blue-400 synthwave:text-pink-500'}`}>
                      {mode.name}
                    </h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mc:text-gray-400 tank:text-[#8a9a70] cyberpunk:text-fuchsia-400/70 retro:text-purple-400/70 error404:text-red-500/70 hacker:text-green-500/70 ocean:text-blue-400/70 synthwave:text-pink-500/70">
                      {mode.desc}
                    </p>
                  </div>
                  {isActive && (
      ...
      <section className="py-24 px-6 relative">
        <div className="max-w-3xl mx-auto">
          <div className="bg-white dark:bg-[#0f0f11] mc:bg-black/60 mc:backdrop-blur-md mc:border-white/10 mc:rounded-none tank:bg-[#11140d]/90 tank:backdrop-blur-lg tank:border-2 tank:border-[#3a5a40] tank:rounded-sm rounded-[2rem] p-8 md:p-12 shadow-xl dark:shadow-none border border-gray-200 dark:border-white/5 cyberpunk:bg-[#0d0221]/80 cyberpunk:backdrop-blur-md cyberpunk:border-fuchsia-500/30 cyberpunk:rounded-none retro:bg-[#2b2b40]/90 retro:backdrop-blur-md retro:border-purple-500/30 retro:rounded-none error404:bg-black error404:border-red-900/50 error404:rounded-none hacker:bg-black hacker:border-green-900/50 hacker:rounded-none transition-colors duration-200">
            <h2 className="font-display text-2xl md:text-3xl font-bold text-gray-900 dark:text-white tank:uppercase tank:tracking-wider tank:text-[#dad7cd] cyberpunk:text-fuchsia-400 retro:text-purple-400 error404:text-red-500 hacker:text-green-500 mb-8 text-center">Contact Support</h2>
            <form action="https://formsubmit.co/alphakoko23@gmail.com" method="POST" className="flex flex-col gap-5">
              <input type="hidden" name="_subject" value="New Support Request from X.KERO.D 'S Site!" />
              <input type="hidden" name="_captcha" value="false" />
              <input type="hidden" name="_template" value="table" />
              
              <div>
                <label htmlFor="name" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mc:text-green-400 tank:text-[#a3b18a] cyberpunk:text-fuchsia-400 retro:text-purple-400 error404:text-red-500 hacker:text-green-500 mb-2">Name</label>
                <input type="text" name="name" id="name" required className="w-full px-4 py-3 rounded-xl mc:rounded-none tank:rounded-sm bg-gray-50 dark:bg-[#161618] mc:bg-black/40 tank:bg-[#1a2315] border border-gray-200 dark:border-white/5 mc:border-white/20 tank:border-[#3a5a40] focus:border-indigo-500 dark:focus:border-indigo-500 mc:focus:border-green-500 tank:focus:border-[#a3b18a] focus:ring-2 focus:ring-indigo-500/20 mc:focus:ring-green-500/20 tank:focus:ring-[#a3b18a]/20 cyberpunk:rounded-none cyberpunk:bg-[#1a0533]/50 cyberpunk:border-fuchsia-900/50 cyberpunk:focus:border-fuchsia-500 cyberpunk:focus:ring-fuchsia-500/20 cyberpunk:text-fuchsia-200 retro:rounded-none retro:bg-[#363650]/50 retro:border-purple-900/50 retro:focus:border-purple-500 retro:focus:ring-purple-500/20 retro:text-purple-200 error404:rounded-none error404:bg-black error404:border-red-900/50 error404:focus:border-red-500 error404:focus:ring-red-500/20 error404:text-red-500 hacker:rounded-none hacker:bg-black hacker:border-green-900/50 hacker:focus:border-green-500 hacker:focus:ring-green-500/20 hacker:text-green-500 text-gray-900 dark:text-white outline-none transition-all" placeholder="Your Name" />
              </div>
              <div>
                <label htmlFor="email" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mc:text-green-400 tank:text-[#a3b18a] cyberpunk:text-fuchsia-400 retro:text-purple-400 error404:text-red-500 hacker:text-green-500 mb-2">Email</label>
                <input type="email" name="email" id="email" required className="w-full px-4 py-3 rounded-xl mc:rounded-none tank:rounded-sm bg-gray-50 dark:bg-[#161618] mc:bg-black/40 tank:bg-[#1a2315] border border-gray-200 dark:border-white/5 mc:border-white/20 tank:border-[#3a5a40] focus:border-indigo-500 dark:focus:border-indigo-500 mc:focus:border-green-500 tank:focus:border-[#a3b18a] focus:ring-2 focus:ring-indigo-500/20 mc:focus:ring-green-500/20 tank:focus:ring-[#a3b18a]/20 cyberpunk:rounded-none cyberpunk:bg-[#1a0533]/50 cyberpunk:border-fuchsia-900/50 cyberpunk:focus:border-fuchsia-500 cyberpunk:focus:ring-fuchsia-500/20 cyberpunk:text-fuchsia-200 retro:rounded-none retro:bg-[#363650]/50 retro:border-purple-900/50 retro:focus:border-purple-500 retro:focus:ring-purple-500/20 retro:text-purple-200 error404:rounded-none error404:bg-black error404:border-red-900/50 error404:focus:border-red-500 error404:focus:ring-red-500/20 error404:text-red-500 hacker:rounded-none hacker:bg-black hacker:border-green-900/50 hacker:focus:border-green-500 hacker:focus:ring-green-500/20 hacker:text-green-500 text-gray-900 dark:text-white outline-none transition-all" placeholder="your@email.com" />
              </div>
              <div>
                <label htmlFor="problem" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mc:text-green-400 tank:text-[#a3b18a] cyberpunk:text-fuchsia-400 retro:text-purple-400 error404:text-red-500 hacker:text-green-500 mb-2">Problem</label>
                <textarea name="problem" id="problem" required rows={4} className="w-full px-4 py-3 rounded-xl mc:rounded-none tank:rounded-sm bg-gray-50 dark:bg-[#161618] mc:bg-black/40 tank:bg-[#1a2315] border border-gray-200 dark:border-white/5 mc:border-white/20 tank:border-[#3a5a40] focus:border-indigo-500 dark:focus:border-indigo-500 mc:focus:border-green-500 tank:focus:border-[#a3b18a] focus:ring-2 focus:ring-indigo-500/20 mc:focus:ring-green-500/20 tank:focus:ring-[#a3b18a]/20 cyberpunk:rounded-none cyberpunk:bg-[#1a0533]/50 cyberpunk:border-fuchsia-900/50 cyberpunk:focus:border-fuchsia-500 cyberpunk:focus:ring-fuchsia-500/20 cyberpunk:text-fuchsia-200 retro:rounded-none retro:bg-[#363650]/50 retro:border-purple-900/50 retro:focus:border-purple-500 retro:focus:ring-purple-500/20 retro:text-purple-200 error404:rounded-none error404:bg-black error404:border-red-900/50 error404:focus:border-red-500 error404:focus:ring-red-500/20 error404:text-red-500 hacker:rounded-none hacker:bg-black hacker:border-green-900/50 hacker:focus:border-green-500 hacker:focus:ring-green-500/20 hacker:text-green-500 text-gray-900 dark:text-white outline-none transition-all resize-none" placeholder="Describe your problem here..."></textarea>
              </div>
              <button type="submit" className="mt-4 w-full py-4 rounded-xl mc:rounded-none tank:rounded-sm bg-indigo-600 hover:bg-indigo-500 mc:bg-green-600 mc:hover:bg-green-500 tank:bg-[#3a5a40] tank:hover:bg-[#588157] cyberpunk:rounded-none cyberpunk:bg-fuchsia-600 cyberpunk:hover:bg-fuchsia-500 retro:rounded-none retro:bg-purple-600 retro:hover:bg-purple-500 error404:rounded-none error404:bg-red-600 error404:hover:bg-red-500 hacker:rounded-none hacker:bg-green-600 hacker:hover:bg-green-500 text-white font-medium transition-all shadow-md tank:uppercase tank:tracking-wider">
      ...
      <footer className="py-12 border-t border-gray-200 dark:border-white/5 mc:border-white/10 tank:border-[#3a5a40]/50 vertigo:border-white/10 cyberpunk:border-fuchsia-500/30 cyberpunk:bg-[#0d0221]/80 retro:border-purple-500/30 retro:bg-[#2b2b40]/90 bg-white/50 dark:bg-[#050505]/50 mc:bg-black/60 tank:bg-[#11140d]/80 vertigo:bg-white/5 backdrop-blur-md transition-colors duration-200">
        <div className="max-w-7xl mx-auto px-6 flex flex-col items-center justify-center gap-6">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl mc:rounded-none tank:rounded-sm vertigo:rounded-2xl cyberpunk:rounded-none cyberpunk:bg-fuchsia-600 cyberpunk:shadow-fuchsia-500/20 retro:rounded-none retro:bg-purple-600 retro:shadow-purple-500/20 error404:rounded-none error404:bg-red-900 error404:shadow-red-500/20 hacker:rounded-none hacker:bg-green-900 hacker:shadow-green-500/20 bg-indigo-600 dark:bg-indigo-500 mc:bg-green-600 tank:bg-[#3a5a40] vertigo:bg-cyan-500 flex items-center justify-center shadow-lg shadow-indigo-500/20 mc:shadow-green-500/20 tank:shadow-[#3a5a40]/20 vertigo:shadow-cyan-500/20">
              <span className="font-display font-bold text-white error404:text-red-500 hacker:text-green-500 text-xl tracking-tighter">X</span>
            </div>
            <span className="font-display font-bold text-xl tracking-tight text-gray-900 dark:text-white mc:text-2xl tank:text-2xl tank:uppercase tank:tracking-widest tank:text-[#dad7cd] vertigo:text-2xl vertigo:tracking-tighter vertigo:font-medium cyberpunk:text-fuchsia-400 cyberpunk:tracking-widest retro:text-purple-400 retro:tracking-widest error404:text-red-500 hacker:text-green-500">X.KERO.D 'S Site</span>
          </div>
          <p className="text-gray-500 dark:text-gray-400 mc:text-green-500/70 tank:text-[#a3b18a]/70 vertigo:text-cyan-100/50 cyberpunk:text-fuchsia-400/70 retro:text-purple-400/70 error404:text-red-500/70 hacker:text-green-500/70 text-sm text-center max-w-md">
          ...
          <div className="flex items-center gap-4 mt-2">
            ...
            <a href="https://discord.gg/ZXg9eUdc7f" target="_blank" rel="noreferrer" className="p-2 rounded-full mc:rounded-none tank:rounded-sm vertigo:rounded-xl cyberpunk:rounded-none cyberpunk:hover:text-fuchsia-400 cyberpunk:hover:bg-fuchsia-900/30 retro:rounded-none retro:hover:text-purple-400 retro:hover:bg-purple-900/30 text-gray-400 hover:text-gray-900 dark:hover:text-white mc:hover:text-green-400 tank:hover:text-[#dad7cd] vertigo:hover:text-cyan-400 hover:bg-gray-100 dark:hover:bg-white/5 mc:hover:bg-green-900/30 tank:hover:bg-[#3a5a40]/30 vertigo:hover:bg-white/10 transition-all">
              <MessageSquare className="w-5 h-5" />
            </a>
          </div>
          ...
          <p className="text-gray-400 dark:text-gray-500 mc:text-green-600/50 tank:text-[#588157]/70 vertigo:text-cyan-500/40 cyberpunk:text-fuchsia-600/50 retro:text-purple-600/50 error404:text-red-500/50 hacker:text-green-500/50 text-xs">
            &copy; {new Date().getFullYear()} X.KERO.D 'S Site. All rights reserved.
          </p>
        </div>
      </footer>
    </div>
